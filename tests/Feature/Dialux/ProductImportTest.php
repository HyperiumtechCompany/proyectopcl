...
test('photometry repair restores legacy ldt data and synchronizes placed fixtures without losing edits', function () {
    ...
    expect(array_map('floatval', $product->photometric_web['c_angles']))->toBe([0.0])
        ...
        // Revierte Ronda 21m: candela absoluta tal cual llega (500 cd), sin
        // re-normalizar a cd/1000lm — ver comentario en `buildPolarSvg()`.
        ->and($product->report_assets['polar_svg'])->toContain('Imax 500.0 cd')
...
test('buildPolarSvg muestra el flujo total y dibuja C0/C180 + C90/C270 cuando hay dos planos reales (Ronda 21g)', function () {
    ...
    // Revierte Ronda 21m: candela absoluta tal cual llega (1000 cd), sin
    // re-normalizar a cd/1000lm.
    expect($svg)->toContain('Imax 1,000.0 cd · Φ 2,014 lm')
...
test('buildPolarSvg con un solo plano C mantiene la curva azul única, sin leyenda (sin regresión visual)', function () {
    ...
    // Revierte Ronda 21m: candela absoluta tal cual llega (802 cd), sin
    // re-normalizar a cd/1000lm.
    expect($svg)->toContain('Imax 802.0 cd · Φ 2,014 lm')
