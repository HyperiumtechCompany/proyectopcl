...
| voltage-drop-red-v2 | CNE-Utilización (Perú), Regla 050-102 (citada en el comentario del backend) | no confirmada — edición y numeral sin verificar contra el texto oficial | `app/Services/Dialux/V2/ElectricalNetworkService.php` (default de la red), `resources/js/pages/dialux/v2/electrical-network/components/ElectricalPropertiesPanel.tsx` (nota visible) | todos (red multimódulo v2) | 2.5 % por alimentador y 4 % acumulado total (alimentador + circuito); editables por proyecto | pending-confirmation | |

**Discrepancia abierta, sigue sin resolver (confirmada en Fase 3 del plan de agentes; ampliada 2026-09-25)**: existen **cuatro** series de valores en el sistema — las tres de abajo más la de la red v2 (2.5 % alimentador / 4 % total, fila `voltage-drop-red-v2`; la interfaz la rotula "dentro del límite configurado", nunca "conforme"). Las tres originales — la del Excel origen (4 %/2.5 %/1 % por nivel del árbol, en `plan_caida_tension.md`), la sembrada en base de datos (2.0–3.0 % plano por `circuit_type`×`installation_category`, sin cascada), y la que se recuerda de forma general del CNE-Perú (2.5 %+2.5 % = 5 % total). **Ninguna de las cuatro debe asumirse correcta** hasta que el especialista indique cuál rige para este proyecto, bajo qué edición, y si aplica por tramo o en cascada acumulada (ver `.claude/agents/dialux-electrical-reviewer.md`, que ya implementa el cálculo de `cumulativeVoltageDropPct` pero sin umbral total confirmado).
...
## 5c. Emplazamiento exterior: rampas, muros de contención y cercos (dominio: geometría)

> Agregado 2026-09-18 al implementar `resources/js/pages/dialux/v2/site/domain/siteNorms.ts`. Los textos se leyeron de los PDF oficiales descargados (A.120: gob.pe; E.050: RM 406-2018-VIVIENDA, El Peruano 03-dic-2018), no de memoria. **Ninguna fila está confirmada por un especialista**: el código solo muestra "dentro del valor de referencia" o "revisar", nunca "cumple".

| id | fuente | edición_conocida | en_código | tipoProyecto | valor_referencial | estado | verificado_por |
|---|---|---|---|---|---|---|---|
| rampa-pendiente-a120-2019 | RNE A.120 Art. 6 b) | RM 072-2019-VIVIENDA | — (versión menos estricta, no usada) | todos | Hasta 0.25 m: 12 %; 0.26-0.75 m: 10 %; 0.76-1.20 m: 8 % (tabla del PDF de 2019, filas siguientes no transcritas) | pending-confirmation | |
| rampa-pendiente-a120-posterior | RNE A.120 Art. 6 b), c) | versión posterior (PDF gob.pe con firma digital 2021-07-19; una nota de prensa la asocia a RM 075-2023-VIVIENDA — **sin confirmar cuál rige**) | `v2/site/domain/siteNorms.ts::RAMP_NORM` | todos | Desnivel hasta 0.30 m: 10 %; 0.31-0.72 m: 8 %; longitud máxima de tramo 9.00 m con descanso de 1.50 m | pending-confirmation | |
| rampa-ancho-a120 | RNE A.120 Art. 6 a) | ídem | `siteNorms.ts::RAMP_NORM.minWidthM` | todos | Ancho mínimo 1.00 m incluyendo pasamanos y barandas; pendiente transversal < 2 % (Art. 6 e) | pending-confirmation | |
| rampa-baranda-a120 | RNE A.120 Art. 6-7 | ídem | `siteNorms.ts::RAMP_NORM.handrailHeightM`; `SiteBuilder3D.buildFlightRamp` (baranda 3D) | todos | Doble pasamanos: uno a 0.85-0.90 m y otro 0.25 m más abajo; descanso con espacio de maniobra 1.50 × 1.50 m (aforo > 500) o 1.20 m de diámetro | pending-confirmation | |
| contencion-ems-e050 | RNE E.050 (RM 406-2018-VIVIENDA) num. 6.2.1 g) | 2018 | `siteNorms.ts::RETAINING_NORM.emsRequiredAboveM` | todos | EMS obligatorio para muros de contención de más de 2.00 m y para cercos perimétricos en terrenos sin EMS; también para edificación adyacente a taludes (h) | pending-confirmation | |
| contencion-fs-e050 | RNE E.050 39.13.6, 30.3, 39.13.8 | 2018 | `siteNorms.ts::RETAINING_NORM` | todos | Factor de seguridad mínimo de muro y de talud: 1.50 estático / 1.25 pseudo-dinámico; muros con drenaje; diseño según E.060 (39.13.2) | pending-confirmation | |
| escalera-exterior-referencia | RNE A.010 (valores ya citados en el código, preset vivienda) + A.120 Art. 6-7 | no confirmada | `hooks/stairNorms.ts` (housing) y `v2/site/domain/siteNorms.ts::STAIR_NORM` | todos | Contrahuella máx. 0.175 m, huella mín. 0.28 m, ancho mín. 0.90 m, descanso cada 18 peldaños (valores del código, sin verificar contra el texto oficial); A.120 solo fija uniformidad de pasos/contrapasos, pasamanos a 0.85-0.90 m (Art. 7) y señalización podotáctil al inicio y final (Art. 6 h). Ver discrepancia A.010 vs A.020 en §6 | pending-confirmation | |
| rampa-descanso-llegada | RNE A.120 Art. 6 d), g) | ídem | `rampLayout.ts` (arrivalLandingM, 1.50 m por defecto) | todos | Arranque y entrega de rampas con superficie plana y horizontal, espacio de maniobra 1.50 × 1.50 m (aforo > 500) o 1.20 m de diámetro; tramos paralelos con descanso que abarque ambos, fondo ≥ 1.50 m | pending-confirmation | |
| iluminancia-exterior | EN 12464-2:2014 (Tablas 5.1 y 5.9) y EN 13201-2:2015 (clases P y C) | 2014 / 2015 — valores de conocimiento general, sin verificar contra el texto oficial | `v2/site/domain/exteriorNormCatalog.ts` (región "Exterior" del emplazamiento, 2026-09-25); cálculo con el motor V1 en `siteLightingCalculation.ts` | todos | Ēm y Uo (EN 12464-2), Ē y Emín (clases P), Ē y Uo 0,40 (clases C). Clases M (luminancia) NO incluidas. La interfaz compara numéricamente ("≥ norma"/"< norma") y declara la fuente como pendiente de confirmar. Las regiones Europa/EE.UU./Perú siguen siendo catálogos de INTERIORES, con aviso en exteriores | pending-confirmation | |
| cerco-altura-tipo | **sin fuente cargada** | — | — | todos | Altura máxima, tipo (ciego/reja) y retiros de cercos: dependen de la ordenanza municipal y la zonificación del predio. El código NO valida ni cita ningún valor | no configurada | |
| talud-angulo | **sin fuente cargada** | — | `v2/site` (`taludAngleDeg`, 75° por defecto) | todos | El ángulo de talud del modelo 3D es geométrico y editable; no es un análisis de estabilidad ni sale de una norma | no configurada | |
