...
function getFixtureColors(fx: Fixture, isSelected: boolean): {
    fill: string;
    stroke: string;
    glow: string;
    label: string;
    ...
    if (isSelected) {
        return { fill: '#fde68a', stroke: '#f59e0b', glow: '#f59e0b', label: '#fde68a' };
    }
    switch (fx.fixtureType) {
        case 'spot':
            return { fill: '#fcd34d', stroke: '#d97706', glow: '#d97706', label: '#fbbf24' };
        case 'panel':
            return { fill: '#e0f2fe', stroke: '#38bdf8', glow: '#38bdf8', label: '#7dd3fc' };
        case 'tube':
            return { fill: '#d1fae5', stroke: '#34d399', glow: '#10b981', label: '#6ee7b7' };
        case 'strip':
            return { fill: '#ede9fe', stroke: '#a78bfa', glow: '#8b5cf6', label: '#c4b5fd' };
        case 'pendant':
            return { fill: '#fee2e2', stroke: '#f87171', glow: '#ef4444', label: '#fca5a5' };
        case 'surface':
            return { fill: '#fef3c7', stroke: '#f59e0b', glow: '#d97706', label: '#fde68a' };
        default: // recessed
            return { fill: '#fbbf24', stroke: '#d97706', glow: '#d97706', label: '#fbbf24' };
    }
}
...
export const OverlayFixtures = memo(function OverlayFixtures({
    fixtures, selectedId, zoom, onSelect, screenPoint, screenDistance,
}: Props) {
    ...
    return (
        <g className="overlay-fixtures">
            <defs>
                ...
                <filter id="glow-fixture-round" x="-50%" y="-50%" width="200%" height="200%">
                ...
                <filter id="glow-fixture-panel" x="-20%" y="-20%" width="140%" height="140%">
                    ...
                    >
                        ...
                        {shape === 'round' ? (
                            ...
                            <rect
                                ...
                                rx={shape === 'cylindrical' ? safeNum(dims.h / 2) : safeNum(Math.min(3, dims.w * 0.1))}
                                ry={shape === 'cylindrical' ? safeNum(dims.h / 2) : safeNum(Math.min(3, dims.h * 0.1))}
                                fill={colors.fill}
                                stroke={colors.stroke}
                                strokeWidth={isSelected ? 2 : 1.5}
                                filter={`url(#${filterId})`}
                        ...
                        {shape === 'cylindrical' && (
                            <line
                                ...
                                y2={safeNum(fp.y)}
                                stroke={colors.stroke}
                                strokeWidth={safeNum(Math.max(1, dims.h * 0.25))}
                                strokeLinecap="round"
                                opacity={0.6}
                            />
                        )}
                        ...
                        {(shape === 'square' || shape === 'rectangular') && zoom > 0.6 && (
                            <>
                                <line
                        ...
                        <text
                            ...
                            fill="#0d0f14"
                            fontSize={safeNum(Math.min(dims.w * 0.55, dims.h * 0.85, 14))}
                            fontWeight="bold"
                            pointerEvents="none"
