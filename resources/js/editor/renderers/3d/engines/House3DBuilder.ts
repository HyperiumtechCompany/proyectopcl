...
export class House3DBuilder {
    ...
    private initMaterials() {
        ...
        this.matPasadizoSlab = new StandardMaterial('mat_pasadizo_slab', this.scene);
    ...
    private expandPolygonShape(shape: Vector3[], amount: number): Vector3[] {
        ...
        return shape.map((v, i) => {
            ...
            const d1x = v.x - prev.x, d1z = v.z - prev.z;
            const d2x = next.x - v.x,  d2z = next.z - v.z;
            ...
            if (Math.abs(cosThetaOver2) < 0.1) cosThetaOver2 = 0.1 * Math.sign(cosThetaOver2) || 0.1;
        ...
        if (room.roomType !== 'corridor') {
            ...
            for (let i = 0; i < verts.length; i++) {
                ...
                const wallBox = MeshBuilder.CreateBox(
                    ...
                    { width: segLen + WALL_THICKNESS, height: ceilingHeight, depth: WALL_THICKNESS },
                    this.scene,
                );
            ...
            if (room.roomType !== 'corridor') {
                ...
                for (let i = 0; i < verts.length; i++) {
                    ...
                    const roofEdge = MeshBuilder.CreateBox(
                        ...
                        { width: segLen + WALL_THICKNESS, height: 0.20, depth: WALL_THICKNESS },
                        this.scene,
                    );
                    roofEdge.position.set(cx, ceilingHeight + 0.10, cz);
    ...
    private buildRoom(
        ...
        const EXTERIOR_WALL_THICKNESS = 0.20; // 20 cm — pared exterior estándar
        ...
        if (room.roomType === 'corridor') {
            ...
            const corridorShape = this.expandPolygonShape(shape, EXTERIOR_WALL_THICKNESS / 2);
            ...
            try {
                ...
                if (!this.warnedInvalidRooms.has(room.id)) {
                    this.warnedInvalidRooms.add(room.id);
                    console.warn(`Pasadizo ${room.id}: polygon mesh failed, using box fallback`);
                }
                const bounds = this.getRoomBounds(room);
                const w = Math.max(0.1, bounds.maxX - bounds.minX) + EXTERIOR_WALL_THICKNESS;
                const d = Math.max(0.1, bounds.maxY - bounds.minY) + EXTERIOR_WALL_THICKNESS;
                ...
                fallbackSlab.position.set(bcx, ceilingHeight + SLAB_THICKNESS / 2, bcz);
        ...
        if (showRoof) {
            try {
                ...
                const ceilingShape = this.expandPolygonShape(shape, EXTERIOR_WALL_THICKNESS / 2);
    ...
    private getAperturesForSegment(
        ...
        const checkAperture = (ap: any, type: 'window' | 'door') => {
            ...
            if (!wall) return;
            const centerOffset = type === 'window' ? ap.offsetAlongWall : ap.offsetAlongWall + ap.width / 2;
    ...
    private buildWallSegment(
        ...
        if (allAps.length === 0) {
            ...
            this.positionWallBoxOffset(m, [v1, v2], segLen / 2 + centerShift, angleRad);
    ...
    private buildWall(
        ...
        for (let i = 0; i < vertices.length - 1; i++) {
            ...
            for (const w of allWindows.filter(win => win.wallId === wall.id)) {
                const wStart = w.offsetAlongWall - (w.width || 0) / 2;
                const wEnd = w.offsetAlongWall + (w.width || 0) / 2;
                const segEndOffset = segStartOffset + segLen;

                if (wStart < segEndOffset - 0.01 && wEnd > segStartOffset + 0.01) {
            ...
            const segmentDoors: any[] = [];
            for (const d of _allDoors.filter(door => door.wallId === wall.id)) {
                const dStart = d.offsetAlongWall;
                const dEnd = d.offsetAlongWall + d.width;
                const segEndOffset = segStartOffset + segLen;
