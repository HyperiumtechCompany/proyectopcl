...
const Cisterna = ({ initialData, canEdit, editMode, onChange, globalDemandaTotal }) => {
    ...
    const [largo, setLargo] = useState(initialData?.largo ?? 4.40);
    const [ancho, setAncho] = useState(initialData?.ancho ?? 2.70);
    const [alturaUtil, setAlturaUtil] = useState(initialData?.alturaUtil ?? 1.90);
    const [bordeLibre, setBordeLibre] = useState(initialData?.bordeLibre ?? 0.50);
    const [nivelagua, setNivelagua] = useState(initialData?.nivelagua ?? 0.65);
    ...
    // Cálculos
    const volumenCisterna = useMemo(() => {
        return (3 / 4) * (consumoDiario / 1000);
    }, [consumoDiario]);

    const volumenTotal = volumenCisterna;

    const alturaAguaMin = useMemo(() => {
        if (largo === 0 || ancho === 0) return 0;
        return volumenCisterna / (largo * ancho);
    }, [volumenCisterna, largo, ancho]);
    ...
    const calculateCisternaData = useCallback(() => {
        ...
        const volumencisternaReal = (volumenCalculado > 30) ? 0.15 : 0.10;
        ...
        const nivel4 = round(nivel3 - volumencisternaReal);
    ...
    const sendDataUpdate = useCallback(() => {
        ...
        const event = new CustomEvent('cisterna-updated', { detail: data });
        document.dispatchEvent(event);

        if (onChange) {
            onChange(data);
        }
    }, [consumoDiario, largo, ancho, alturaUtil, bordeLibre, nivelagua, alturaTecho, volumenCisterna, volumenCalculado, alturaTotal, calculateCisternaData, onChange]);
    ...
    // Dibujo del Canvas
    const drawCisternaNative = useCallback(() => {
        ...
        const cisternaWidth = (parseFloat(largo) || 0) * scale;
        const cisternaHeight = (alturaTotal || 0) * scale;
        ...
        ctx.fillText('TECHO', startX + cisternaWidth / 2, startY + ((parseFloat(alturaTecho) || 0) * scale) / 2);
        ctx.fillText('BORDE LIBRE', startX + cisternaWidth / 2, techoY + ((parseFloat(bordeLibre) || 0) * scale) / 2);
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`${parseFloat(largo)} m`, startX + cisternaWidth / 2, startY + cisternaHeight + 35);
        ...
        const infoTexts = [
            `Vol. Requerido: ${volumenCisterna.toFixed(2)} m³`,
    ...
    }, [largo, ancho, alturaUtil, bordeLibre, alturaTecho, alturaTotal, volumenCisterna, volumenCalculado]);
    ...
    }, [canvasReady, drawCisternaNative]);
    ...
    return (
        <div className="w-full p-4">
                ...
                <div className="w-full px-2 py-2">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-slate-800">2. CALCULO DEL ALMACENAMIENTO - CISTERNA</h1>
            ...
            <main className="max-w-full mx-auto px-2 py-2 space-y-2">
                <div className="p-2">
                    {/* Input Parameters */}
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <div className="bg-slate-50 rounded-lg p-6">
                                <h3 className="text-lg font-semibold text-slate-800 mb-4">2.1.1. CÁLCULO DE VOLUMEN</h3>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input type="number" step="0.01" value={consumoDiario || ''}
                                            className="w-full px-4 py-2 text-gray-950 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-slate-100"
                                            readOnly />
                                    ...
                                    <div className="bg-blue-50 p-4 rounded-lg">
                                        <p className="text-sm text-blue-800 mb-2">VOL. DE CISTERNA = 3/4 x CONSUMO DIARIO TOTAL</p>
                                        <p className="text-lg font-semibold text-blue-900">Vol. De Cisterna = {(volumenCisterna || 0).toFixed(2)} m³</p>
                            ...
                            <div className="bg-slate-50 rounded-lg p-6">
                                <h3 className="text-lg font-semibold text-slate-800 mb-4">Dimensiones de la Cisterna</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" step="0.01" value={largo} onChange={(e) => setLargo(e.target.value)} disabled={!editMode}
                                            className="w-full px-3 py-2 text-gray-950 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-slate-100" />
                                    </div>
                                    <div>
                                ...
                                <div className="mt-4 p-4 bg-green-50 rounded-lg">
                                    <p className="text-lg font-bold text-green-800">VOLUMEN DE CISTERNA = {(volumenCalculado || 0).toFixed(2)} m³</p>
                                    {volumenCalculado < volumenCisterna && (
                        ...
                        <div className="space-y-6">
                            <div className="bg-white border border-slate-200 rounded-lg p-6">
                                <h3 className="text-lg font-semibold text-slate-800 mb-2">Diagrama de Cisterna</h3>
                            ...
                            <div className="bg-slate-50 rounded-lg p-6">
                                <h3 className="text-xs font-semibold text-slate-800 mb-4">2.1.2. DIMENSIONES DE LA CISTERNA</h3>
                                <div className="space-y-3">
                                    <div className="flex justify-between">
                                        <span className="text-slate-600">ANCHO (A): {ancho} m</span>
                                        <span className="text-xs">Ancho de la Cisterna</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600">LARGO (L): {largo} m</span>
                                        <span className="text-xs">Largo de la Cisterna</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600">ALTURA DE AGUA (H): {alturaUtil} m</span>
                                        <span className="text-xs">Altura de agua de la Cisterna</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600">Borde Libre (bl): {bordeLibre} m</span>
                                        <span className="text-xs">Altura de la Cisterna</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-600">Altura total (HT): {alturaTotal} m</span>
                                        <span className="text-xs">Altura total de la Cisterna</span>
                                    </div>
