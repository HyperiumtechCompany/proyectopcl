...
const MODULO_SUFIJO = '(RED DE COCINA-CUARTO DE LIMPIEZA)';

const RedesInteriores = ({ initialData, canEdit, editMode, onChange, maximaDemandaData, tuberiasrdData }) => {
    ...
    const crearFilaVacia = useCallback((grado = 'inicial') => {
        ...
        const diam = ''; // En redes interiores suele empezar vacío
        const row = {
        ...
        // Si hay un diámetro por defecto o al crear, calculamos
        if (diam) {
    ...
    const getColumns = useCallback((grado, moduloId) => {
        ...
        return [
            {
                title: 'Segmento', field: 'segmento', editor: editable ? 'input' : false, headerVertical: true, width: 65,
    ...
    const initTabulator = useCallback((grado, moduloId, data = [], attempt = 0) => {
        const id = `tabulator-table-${grado}-${moduloId}`;
    ...
    // Efecto 2: grades cambia → crear módulo inicial
    useEffect(() => {
        setTables(prev => {
            ...
            <header className="bg-white/80 backdrop-blur-lg border-b border-slate-200/60 shadow-lg sticky top-12 z-50">
                <div className="max-w-7xl mx-auto px-6 py-4">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-800">7. CÁLCULO DE REDES INTERIORES</h1>
                            <p className="text-sm text-slate-600">Red hidráulica interior</p>
                        </div>
                    </div>
                </div>
            </header>
                ...
                {Object.entries(tables).filter(([grado]) => grades[grado]).map(([grado, table]) => (
                    <div key={grado} className="bg-white/90 rounded-xl shadow-lg border border-slate-200/60 overflow-hidden">
                        <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white">
                            ...
                            <div className="flex items-center space-x-3">
                                {mode === 'edit' && (
                                    <button onClick={() => addModulo(grado)} className="bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-sm flex items-center gap-1">
                            ...
                            <div className="bg-white">
                                {table.modules.map(modulo => (
                                    <div key={modulo.id} className="mb-8 border border-slate-200 rounded-xl shadow-inner bg-slate-50 mx-3 mt-3">
                                        ...
                                        <div className="p-3">
                                            <div id={`tabulator-table-${grado}-${modulo.id}`} className="w-full overflow-x-auto border rounded-lg shadow-inner min-h-[160px]"></div>
                                            {mode === 'edit' && (
                                                <div className="flex justify-end mt-3">
                                                    <button onClick={() => addRow(grado, modulo.id)} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm flex items-center gap-1">
