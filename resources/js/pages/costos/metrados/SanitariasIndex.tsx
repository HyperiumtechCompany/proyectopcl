    ...
    try {
      ...
      const results = await Promise.all(
        sheetsToSave.map((s) => {
          ...
          if (isRes) url = `/costos/${projectId}/metrado-sanitarias/resumen`;
          else if (isExt) url = `/costos/${projectId}/metrado-sanitarias/exterior`;
          else if (isCis) url = `/costos/${projectId}/metrado-sanitarias/cisterna`;
          else {
            // "Módulo X"
...
export default function SanitariasIndex() {
  const { project, config, modulos, exterior, cisterna, resumen } = usePage<SanitariasPageProps>().props;
  ...
  const resumenRows = useMemo(() => {
    const generated = buildSanitariasResumenRows(modulos, exterior, cisterna, moduleCount, resumen ?? []);
  ...
  const syncResumen = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const newRows = buildSanitariasResumenRows(mods, ext, cis, moduleCount, previousResumenRows);
