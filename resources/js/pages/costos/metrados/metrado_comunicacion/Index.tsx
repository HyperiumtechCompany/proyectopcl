...
export default function Index() {
    ...
    const handleCreate = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(comunicacionRoutes.store.url(), { name, project_name: projectName }, {
    ...
    const handleJoin = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(comunicacionRoutes.join.url(), { code: code.toUpperCase() }, {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="w-full rounded-lg bg-white px-12 py-6 shadow dark:bg-gray-900">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Hojas de Comunicaciones</h1>
                ...
                {spreadsheets.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                        {spreadsheets.map((sheet) => (
                            <div key={sheet.id} className="group cursor-pointer rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800" onClick={() => router.get(comunicacionRoutes.show.url(sheet.id))}>
                                <div className="p-4">
                                        ...
                                        <button
                                            onClick={(e) => {
                                                ...
                                                if (confirm('¿Eliminar esta hoja?')) {
                                                    router.delete(comunicacionRoutes.destroy.url(sheet.id));
                                                }
                ...
                {showCreate && (
                    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                        <div className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900 dark:ring-1 dark:ring-gray-700">
                            <h2 className="mb-4 text-base font-bold text-gray-800 dark:text-gray-100">Nueva Hoja de Comunicaciones</h2>
                            <form onSubmit={handleCreate} className="space-y-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ej: Metrado Comunicaciones - Edificio A"
