...
import type { MetradoGasSpreadsheetSummary } from '@/types/metrado_gas';
import * as gasRoutes from '@/routes/metrados/gas';
import metradoRoutes from '@/routes/metrados';

interface PageProps {
    spreadsheets: MetradoGasSpreadsheetSummary[];
...
export default function Index() {
    ...
    const handleCreate = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(gasRoutes.store.url(), { name, project_name: projectName }, {
    ...
    const handleJoin = (e: React.FormEvent) => {
        e.preventDefault();
        setProcessing(true);
        router.post(gasRoutes.join.url(), { code: code.toUpperCase() }, {
    ...
    // Función para obtener ícono según tipo de gas (si está disponible en el summary)
    const getGasIcon = (type?: string) => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            <div className="w-full rounded-lg bg-white px-12 py-6 shadow dark:bg-gray-900">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Metrados de Gas</h1>
                ...
                {spreadsheets.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-gray-300 bg-white py-16 text-center dark:border-gray-700 dark:bg-gray-900">
                        <p className="mb-3 text-sm text-gray-500 dark:text-gray-400">No tienes metrados de gas creados todavía.</p>
                        <button
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                        {spreadsheets.map((sheet) => (
                            <div key={sheet.id} className="group cursor-pointer rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800" onClick={() => router.get(gasRoutes.show.url(sheet.id))}>
                                <div className="p-4">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="text-xl" role="img" aria-label="gas type">
                                                    {getGasIcon(sheet.gas_type)}
                                                </span>
                                            ...
                                            {sheet.project_name && (
                                                <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 ml-7">{sheet.project_name}</p>
                                            )}
                                            {sheet.gas_type && (
                                                <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 ml-7">
                                                    {sheet.gas_type === 'gn' ? 'Gas Natural' : 
                                                     sheet.gas_type === 'glp' ? 'GLP' : 
                                                     sheet.gas_type === 'gnv' ? 'GNV' : 
                                                     sheet.gas_type === 'industrial' ? 'Industrial' : 
                                                     sheet.gas_type}
                                                </p>
                                            )}
                                        </div>
                                    ...
                                    {sheet.is_owner && (
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                if (confirm('¿Eliminar este metrado de gas?')) {
                                                    router.delete(gasRoutes.destroy.url(sheet.id));
                                                }
                ...
                {showCreate && (
                    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                        <div className="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900 dark:ring-1 dark:ring-gray-700">
                            <h2 className="mb-4 text-base font-bold text-gray-800 dark:text-gray-100">Nuevo Metrado de Gas</h2>
                            <form onSubmit={handleCreate} className="space-y-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="Ej: Metrado Gas - Edificio A"
