// ═══════════════════════════════════════════════════════════════
// ArquitecturaIndex.tsx — Página principal
// ═══════════════════════════════════════════════════════════════
      ...
      const results = await Promise.all(
        sheetsToSave.map((s) => {
          ...
          if (isRes) url = `/costos/${projectId}/metrado-arquitectura/resumen`;
            ...
            if (match) {
              const num = match[1];
              url = `/costos/${projectId}/metrado-arquitectura/modulo/${num}`;
            } else {
...
// ═══════════════════════════════════════════════════════════════
// COMPONENTE PRINCIPAL (ArquitecturaIndex)
// ═══════════════════════════════════════════════════════════════
export default function ArquitecturaIndex() {
  const { project, config, modulos, exterior, cisterna, resumen } = usePage<ArquitecturaPageProps>().props;
  ...
  const resumenRows = useMemo(() => {
    const generated = buildArquitecturaResumenRows(modulos, exterior, cisterna, moduleCount, resumen ?? []);
  ...
  const initialSheets = useMemo(() => {
    ...
    for (let i = 1; i <= moduleCount; i++) {
      const data = injectTemplateIfEmpty(modulos[i] || [], 'arquitectura');
      const sheet = rowsToSheet(data, ALL_COLS, `Módulo ${i}`, currentOrder++);
      sheets.push(sheet);
    }

    sheets.push(rowsToSheet(injectTemplateIfEmpty(exterior || [], 'arquitectura'), ALL_COLS, 'Exterior', currentOrder++));
    sheets.push(rowsToSheet(injectTemplateIfEmpty(cisterna || [], 'arquitectura'), ALL_COLS, 'Cisterna', currentOrder++));
  ...
  const syncResumen = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const newRows = buildArquitecturaResumenRows(mods, ext, cis, moduleCount, previousResumenRows);
