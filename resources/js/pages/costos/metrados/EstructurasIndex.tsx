...
const UNIDAD_OPTIONS = [
  ...
  'm2 v1', 'm2 v2',
  'm3 v1', 'm3 v2', 'm3 v3',
  'kg v1', 'kg v2', 'kg v3', 'kg v4',
  'lt', 'gl', 'pza'
];

const UNIT_TOTAL_COL: Record<string, string> = {
  'm2': 'area', 'm': 'lon', 'ml': 'lon',
  'm3': 'vol', 'lt': 'vol', 'gl': 'vol',
  'kg': 'kg', 'und': 'und', 'pza': 'und',
};

// ═══════════════════════════════════════════════════════════════════════
// CÁLCULO CON VERSIONES
// ═══════════════════════════════════════════════════════════════════════
const calcularConVersion = (
  unidadCompleta: string,
  valores: { largo: number; ancho: number; alto: number; nveces: number; elsim: number; kg: number }
) => {
  const { largo, ancho, alto, nveces, elsim, kg } = valores;
  const [unidadBase, version = 'v1'] = unidadCompleta.split(' ');
  
  const lon = largo * nveces;
  const area = largo * ancho * nveces;
  const vol = largo * ancho * alto * nveces;
  const und = elsim * nveces;
  
  let total = 0;
  
  if (unidadBase === 'm2') {
    if (version === 'v1') total = area;
    else if (version === 'v2') total = area * 1.05;
  }
  else if (unidadBase === 'm3') {
    if (version === 'v1') total = vol;
    else if (version === 'v2') total = vol * 1.10;
    else if (version === 'v3') total = vol * 1.15;
  }
  else if (unidadBase === 'm' || unidadBase === 'ml') {
    if (version === 'v1') total = lon;
    else if (version === 'v2') total = lon * 1.08;
  }
  else if (unidadBase === 'kg') {
    if (version === 'v1') total = kg;
    else if (version === 'v2') total = kg * 1.10;
    else if (version === 'v3') total = kg * 1.18;
    else if (version === 'v4') total = kg * 1.25;
  }
  else if (unidadBase === 'und' || unidadBase === 'pza') {
    if (version === 'v1') total = und;
    else if (version === 'v2') total = und * 1.05;
  }
  else if (unidadBase === 'lt' || unidadBase === 'gl') {
    total = vol;
  }
...
const FORMULA_INPUTS: Record<string, string[]> = {
  'm v1': ['largo', 'nveces'],
  'm v2': ['largo', 'nveces'],
  'ml v1': ['largo', 'nveces'],
  'ml v2': ['largo', 'nveces'],
  'm2 v1': ['largo', 'ancho', 'nveces'],
  'm2 v2': ['largo', 'ancho', 'nveces'],
  'm3 v1': ['largo', 'ancho', 'alto', 'nveces'],
  'm3 v2': ['largo', 'ancho', 'alto', 'nveces'],
  'm3 v3': ['largo', 'ancho', 'alto', 'nveces'],
  'kg v1': ['kg'],
  'kg v2': ['kg'],
  'kg v3': ['kg'],
  'kg v4': ['kg'],
  'und v1': ['elsim', 'nveces'],
  'und v2': ['elsim', 'nveces'],
  'lt': ['largo', 'ancho', 'alto', 'nveces'],
  'gl': ['largo', 'ancho', 'alto', 'nveces'],
  'pza': ['elsim', 'nveces'],
};
...
//pintar formula inputs
const pintarInputsFormula = (
  ls: any,
  sheetOrder: number,
  ri: number,
  unidad: string,
  ...
  const inputs = FORMULA_INPUTS[unidad] || [];
  const ALL_INPUTS = ['elsim', 'largo', 'ancho', 'alto', 'nveces', 'kg'];

  ALL_INPUTS.forEach((key) => {
    const c = COL[key];
    if (c === undefined) return;

    const isActive = inputs.includes(key);
    const currentValue = row[key];

    const cellValue =
      typeof currentValue === 'number'
        ? mkNum(currentValue)
        : mkTxt(String(currentValue ?? ''));
    ...
    ls.setCellValue(
      ...
      {
        ...cellValue,
        ...(isActive ? INPUT_ACTIVE_STYLE : { bg: null, fc: null }),
        bl: 0,
        fs: 10,
      },
      {
...
export default function EstructurasIndex() {
  ...
  const buildResumenRows = useCallback((metradoData: Record<string, any>[]) => {
    type Agg = { code: string; desc: string; und: string; level: number; total: number };
    ...
    return codeOrder.map((code) => {
      const v = byCode[code];
      return { _level: v.level, _kind: 'group', partida: v.code, descripcion: v.desc, unidad: v.und, total: v.total };
    });
  }, []);
  ...
  const doSave = useCallback(async (sheets: any[]) => {
    ...
    try {
      const results = await Promise.all(
        reqs.map((r) => fetch(r.url, { method: 'PATCH', headers, body: JSON.stringify(r.body) }).then((res) => ({ ok: res.ok, status: res.status })))
      );
  ...
  const recalcActiveSheet = useCallback(() => {
    ...
    entries.forEach((e) => {
      ...
      pintarInputsFormula(ls, sheetOrder, ri, unidad, row);
      ...
      const resultado = calcularConVersion(unidad, { largo, ancho, alto, nveces, elsim, kg });
      
      const upd = (key: string, val: number) => {
        if (toNum(row[key]) !== val) { set(ri, key, mkNum(val)); row[key] = val; }
      };
      
      upd('lon', resultado.lon);
      upd('area', resultado.area);
      upd('vol', resultado.vol);
      upd('und', resultado.und);
      
      e.total = resultado.total;
      set(ri, 'total', mkNum(resultado.total));
    });
    ...
    updates.forEach((u, idx) => {
      ls.setCellValue(u.r, u.c, u.v, { order: sheetOrder, isRefresh: idx === updates.length - 1 });
    });
      ...
      changeData.forEach((change: any) => {
        ...
        if (c === COL['unidad'] && r > 0) {
          setTimeout(() => {
            ...
            console.log('🎯 Unidad cambiada:', unidad, 'en fila', r);

            const data = active.data;
            const row = readDataRow(ls.getSheet().data, r);
            row.unidad = unidad;

            pintarInputsFormula(ls, sheetOrder, r, unidad, row);
            ls.refresh();
          }, 0);
        }
      });
  ...
  const moveBlock = useCallback((direction: 'up' | 'down') => {
    ...
    setTimeout(() => { progUpdateCount.current = Math.max(0, progUpdateCount.current - 1); }, 100);
  }, [recalcActiveSheet]);
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <div className="flex h-[calc(100vh-65px)] w-full flex-col overflow-hidden bg-slate-50 dark:bg-gray-950">
        <header className="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/80 bg-white/92 px-4 py-2 shadow-sm backdrop-blur-md dark:border-gray-800/60 dark:bg-gray-900/92">
          <div className="flex items-center gap-2.5">
            <button type="button" onClick={() => router.get(`/costos/${project.id}`)} className="flex h-7 w-7 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-gray-800 dark:hover:text-gray-200">
              <ChevronLeft className="h-4 w-4" />
            </button>
            
            <div className="leading-tight">
              <p className="text-[13px] font-bold text-slate-900 dark:text-gray-100">Metrado Estructuras</p>
              <p className="text-[9px] font-medium uppercase tracking-wider text-slate-400">{project.nombre}</p>
            </div>
            
            <div className="hidden items-center gap-1 xl:flex">
              {GROUP_PALETTE.slice(0, 4).map((p, i) => (
                <span key={i} className="rounded px-1.5 py-0.5 text-[9px] font-bold" style={{ background: p.bg, color: p.fc }}>N{i + 1}</span>
              ))}
              <span className="rounded px-1.5 py-0.5 text-[9px] font-bold" style={{ background: LEAF_STYLE.bg, color: LEAF_STYLE.fc, border: '1px solid #cbd5e1' }}>Hoja</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <div className="flex items-center gap-1">
              <ActionBtn icon={<FolderPlus className="h-3 w-3" />} label="Grupo" title="Insertar grupo al mismo nivel" style={{ background: GROUP_PALETTE[0].bg, color: '#fff' }} onClick={() => addRow('group', true)} />
              <ActionBtn icon={<Folder className="h-3 w-3" />} label="Sub-grupo" title="Insertar grupo un nivel más profundo" style={{ background: GROUP_PALETTE[2].bg, color: '#fff' }} onClick={() => addRow('group', false)} />
              <ActionBtn icon={<FileText className="h-3 w-3" />} label="Partida" title="Insertar hoja de cálculo" style={{ background: LEAF_STYLE.bg, color: '#1e3a5f', border: '1px solid #cbd5e1' }} onClick={() => addRow('leaf', false)} />
            </div>
            ...
            <Button variant="outline" size="sm" onClick={() => doSave(latestSheets.current)} disabled={saving} className="h-7 gap-1 text-[11px]">
              {saving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3 w-3" />}
              {saving ? 'Guardando…' : 'Guardar'}
            </Button>
            
            <Button variant="outline" size="sm" onClick={handleSyncResumen} disabled={isSyncing || saving} className="h-7 gap-1 text-[11px]">
...
// SUBCOMPONENTES
// ═══════════════════════════════════════════════════════════════════════
function SaveStatus({ saving, error, lastSaved }: { saving: boolean; error: string | null; lastSaved: Date | null }) {
  return (
    <div className="flex items-center rounded-full bg-slate-100/80 px-2.5 py-1 text-[10px] font-semibold dark:bg-gray-800/60">
      {saving ? (
        <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400"><Loader2 className="h-2.5 w-2.5 animate-spin" /> Guardando…</span>
      ) : error ? (
        <span className="flex items-center gap-1 text-red-600 dark:text-red-400"><AlertCircle className="h-2.5 w-2.5" /> {error}</span>
      ) : lastSaved ? (
        <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="h-2.5 w-2.5" />{lastSaved.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' })}</span>
      ) : (
        <span className="flex items-center gap-1 text-slate-400"><Save className="h-2.5 w-2.5" /> Sin cambios</span>
      )}
    </div>
  );
}

function ActionBtn({ icon, label, title, style, onClick }: { icon: React.ReactNode; label: string; title: string; style: React.CSSProperties; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} title={title} className="inline-flex h-7 items-center gap-1 rounded-md px-2 text-[10px] font-bold transition-all hover:opacity-85 active:scale-95" style={style}>
      {icon} {label}
    </button>
  );
}

function ctxItem(text: string, kind: EntryKind, sameLevelAsSelected: boolean, triggerRecalc: () => void, addRow: (k: EntryKind, same: boolean) => void) {
  return { text, type: 'button', onClick: () => { addRow(kind, sameLevelAsSelected); triggerRecalc(); } };
}