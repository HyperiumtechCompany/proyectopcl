...
export function PlazoDisplay({ variant = 'compact', color = 'sky' }: PlazoDisplayProps) {
    ...
    if (variant === 'cards') {
        return (
            <div className="flex items-stretch gap-1.5">
                {/* Días */}
                <div className={`flex items-center gap-1.5 rounded-lg border ${c.border} ${c.bg} px-3 py-2 transition-all hover:scale-[1.02]`}>
                    <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${c.iconBg}`}>
                        <CalendarDays className={`h-4 w-4 ${c.text}`} />
                    </div>
                    <div>
                        <p className="text-xs font-bold tracking-widest text-slate-500 uppercase">Días</p>
                ...
                <div className={`flex items-center gap-1.5 rounded-lg border ${c.border} ${c.bg} px-3 py-2 transition-all hover:scale-[1.02]`}>
                    ...
                    <div>
                        <p className="text-xs font-bold tracking-widest text-slate-500 uppercase">Semanas</p>
                ...
                <div className={`flex items-center gap-1.5 rounded-lg border ${c.border} ${c.bg} px-3 py-2 transition-all hover:scale-[1.02]`}>
                    ...
                    <div>
                        <p className="text-xs font-bold tracking-widest text-slate-500 uppercase">Meses</p>
                ...
                <div className={`flex items-center gap-1.5 rounded-lg border ${c.border} ${c.bg} px-3 py-2 transition-all hover:scale-[1.02]`}>
                    ...
                    <div>
                        <p className="text-xs font-bold tracking-widest text-slate-500 uppercase">Años</p>
                ...
                {(fechaInicio || fechaFin) && (
                    <div className="flex items-center gap-1 rounded-lg border border-slate-700/50 bg-slate-800/30 px-3 py-2">
                        <div className="flex flex-col gap-0.5">
                            {fechaInicio && (
                                <div className="flex items-center gap-1.5">
                                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                                    <span className="text-xs font-semibold text-slate-500">Inicio:</span>
                                    <span className="text-xs font-bold text-slate-300">{fechaInicio}</span>
                                </div>
                            )}
                            {fechaFin && (
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs font-bold text-slate-300">{fechaFin}</span>
                                </div>
                            )}
                        </div>
                    </div>
    ...
    // ── Compact variant ──
    return (
        <div className={`flex items-center gap-1.5 rounded-lg border ${c.border} ${c.bg} px-3 py-1.5`}>
            <div className="flex items-center gap-1.5">
                <Calendar className={`h-3.5 w-3.5 ${c.text}`} />
                <span className="text-xs font-bold tracking-widest text-slate-500 uppercase">Plazo</span>
            </div>
            ...
            <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-slate-500">año(s)</span>
            </div>
                    ...
                    <div className="flex items-center gap-1">
                        {fechaInicio && (
                            <span className="flex items-center gap-1">
                                ...
                                <span className="text-xs text-slate-400">{fechaInicio}</span>
                            </span>
                        )}
                        {fechaFin && (
                            <span className="flex items-center gap-1">
                                <span className="h-1 w-1 rounded-full bg-red-500" />
                                <span className="text-xs text-slate-400">{fechaFin}</span>
                            </span>
                        )}
                    </div>
                </>
