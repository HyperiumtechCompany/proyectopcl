...
const SupervisionRowComponent: React.FC<RowProps> = ({ row, depth, path, onOpenGGDetalle }) => {
    ...
    return (
        <>
            ...
            >
                <td className="p-0">
                    <div className="flex items-center gap-1 py-2 px-4" style={{ paddingLeft: `${depth * 20 + 16}px` }}>
                        {hasChildren ? (
                            <button
                        ...
                        <span className={`text-xs min-w-[20px] font-mono ${getTextColor()}`}>{row.item}</span>
                        <span className={`text-xs truncate ${getTextColor()}`}>{row.descripcion}</span>
                        {isGGSection && (
                ...
                <td className="text-center">
                    {!isSection && !isCalculation && !isCaptura && depth > 1 && (
                        <input
                            ...
                            className="w-16 bg-transparent border-none text-xs text-center text-slate-300 focus:ring-1 focus:ring-sky-500 rounded px-1"
                        />
                    )}
                </td>
                <td className="text-center">
                ...
                <td className="text-right">
                    {row.tipo === 'partida' && (
                        <input
                            ...
                            className="w-24 bg-transparent border-none text-xs text-right text-slate-300 focus:ring-1 focus:ring-sky-500 rounded px-1 mr-4"
                        />
                    )}
                </td>
...
export function SupervisionPanel({ projectId, onSaveSupervision }: SupervisionPanelProps) {
    ...
    React.useEffect(() => {
        if (projectId) {
            ...
            void loadFromDatabase(projectId);
        }
    ...
    return (
        <div className="flex h-full flex-col bg-slate-950 overflow-hidden">
            ...
            <div className="relative overflow-hidden border-b border-slate-800 bg-slate-900/80 p-6 backdrop-blur-md">
                ...
                <div className="relative flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-sm font-black tracking-tight text-white uppercase">Presupuesto de Supervisión</h1>
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <div className="flex flex-col items-end">
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Total Supervisión</span>
            ...
            <div className="grid grid-cols-4 gap-4 p-4 border-b border-slate-800 bg-slate-900/30">
                <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3 flex items-center gap-1.5">
                    <div className="h-8 w-8 rounded bg-sky-500/10 flex items-center justify-center text-sky-500">
                        <DollarSign size={16} />
                    </div>
                    <div>
                        <p className="text-xs font-bold text-slate-500 uppercase">Costo Directo</p>
                        <p className="text-xs font-black text-slate-200 font-mono">S/. {new Intl.NumberFormat('es-PE', { minimumFractionDigits: 2 }).format(rows[2]?.total || 0)}</p>
                    </div>
                </div>
                <div
                    className="rounded-lg border border-amber-500/20 bg-amber-900/10 p-3 flex items-center gap-1.5 cursor-pointer hover:bg-amber-900/20 transition-colors group"
                ...
                >
                    ...
                    <div className="flex-1">
                        <p className="text-xs font-bold text-slate-500 uppercase">Gastos Generales</p>
                        <p className="text-xs font-black text-amber-400 font-mono">S/. {new Intl.NumberFormat('es-PE', { minimumFractionDigits: 2 }).format(rows[3]?.total || 0)}</p>
                    </div>
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3 flex items-center gap-1.5">
                    ...
                    <div>
                        <p className="text-xs font-bold text-slate-500 uppercase">Utilidad (5%)</p>
                        <p className="text-xs font-black text-emerald-400 font-mono">S/. {new Intl.NumberFormat('es-PE', { minimumFractionDigits: 2 }).format(rows[4]?.total || 0)}</p>
                    </div>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3 flex items-center gap-1.5">
                    ...
                    <div>
                        <p className="text-xs font-bold text-slate-500 uppercase">IGV (18%)</p>
                        <p className="text-xs font-black text-purple-400 font-mono">S/. {new Intl.NumberFormat('es-PE', { minimumFractionDigits: 2 }).format(rows[6]?.total || 0)}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
                <table className="w-full border-collapse">
                    <thead className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur-md shadow-lg border-b border-slate-700">
                        <tr>
                            <th className="py-3 px-6 text-left text-xs font-black text-slate-400 uppercase tracking-widest w-[500px]">Concepto</th>
                            <th className="py-3 px-2 text-center text-xs font-black text-slate-400 uppercase tracking-widest w-[80px]">Unidad</th>
                            <th className="py-3 px-2 text-center text-xs font-black text-slate-400 uppercase tracking-widest w-[80px]">Cantidad</th>
                            <th className="py-3 px-2 text-center text-xs font-black text-slate-400 uppercase tracking-widest w-[80px]">Meses</th>
                            <th className="py-3 px-4 text-right text-xs font-black text-slate-400 uppercase tracking-widest w-[120px]">Precio Unit. (S/.)</th>
                            <th className="py-3 px-4 text-right text-xs font-black text-slate-400 uppercase tracking-widest w-[120px]">Subtotal</th>
                            <th className="py-3 px-6 text-right text-xs font-black text-slate-400 uppercase tracking-widest w-[120px]">Total</th>
                        </tr>
                    </thead>
                ...
                <div className="p-8 flex flex-col items-center justify-center text-slate-600">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest mb-1">Cálculos Finalizados</p>
                    <p className="text-xs italic">Los totales de las secciones III, V, VI, VII y VIII se derivan de fórmulas automáticas.</p>
                    <p className="text-xs italic mt-1 text-amber-600/60">Haz clic en la fila IV para editar el detalle de Gastos Generales.</p>
                </div>
            </div>
