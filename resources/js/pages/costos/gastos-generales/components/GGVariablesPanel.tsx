...
import { calcularParcial, formatoMoneda, sumarDecimales } from '../lib/calculos';
    ...
    const handleSyncDuration = () => {
        ...
        const newNodes = nodes.map(node => {
            if (node.tipo_fila === 'detalle' && node.unidad?.toLowerCase().includes('mes')) {
                ...
                const parcial = calcularParcial(node.cantidad_descripcion, cantidad_tiempo, node.participacion, node.precio);
            ...
            <div className="flex items-center justify-between border-b border-slate-700 bg-slate-800/80 px-2 py-1">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col items-end">
                        <span className="text-xs font-semibold text-slate-500 uppercase">Total G.G. Variables</span>
                        <span className="font-mono text-xs font-bold text-amber-400">S/. {fmt(total)}</span>
                    </div>
                </div>
            </div>
                    ...
                    <table className="w-full border-collapse text-left text-xs">
                        <thead className="sticky top-0 z-10 bg-slate-800/95 text-xs font-bold tracking-wider text-slate-400 uppercase backdrop-blur-md">
                            <tr>
                                ...
                                <th className="border-b border-slate-700 p-1 w-16 text-center">UNID.</th>
                                <th className="border-b border-slate-700 p-1 w-20 text-right">CANT. DESC.</th>
                                <th className="border-b border-slate-700 p-1 w-20 text-right">CANT. (N°)</th>
                                <th className="border-b border-slate-700 p-1 w-20 text-right">PARTIC. %</th>
                                <th className="border-b border-slate-700 p-1 w-28 text-right">PRECIO</th>
                                <th className="border-b border-slate-700 p-1 w-28 text-right bg-amber-950/40">PARCIAL</th>
                                ...
                                >
                                    ...
                                    <td className="p-1">
                                        <div className="flex items-center gap-1">
                                            ...
                                            <input
                                                ...
                                                className={`w-full border-none bg-transparent p-1.5 ${getTextColor(node)} ${node.tipo_fila !== 'detalle' || node._fromRemuneraciones ? 'font-semibold text-xs tracking-wide uppercase' : 'text-xs'} focus:bg-slate-700/50 focus:outline-none rounded ${node._fromRemuneraciones ? 'cursor-default' : ''}`}
                                            />
                                        </div>
                                    </td>
                                    ...
                                    <td className="p-1">
                                        {node.tipo_fila === 'detalle' ? (
                                            ...
                                            <div className="text-center text-sky-400 font-bold text-xs">
                                    ...
                                    <td className="p-1">
                                        {node.tipo_fila === 'detalle' && (
                                            <input
                                                ...
                                                readOnly={node._fromRemuneraciones}
                                                className={`w-full border-none bg-transparent p-1 text-right font-mono focus:bg-slate-700/50 focus:outline-none rounded text-xs ${node._fromRemuneraciones ? 'text-emerald-400/80' : 'text-amber-400/80'}`}
                                            />
                                        )}
                                    </td>

                                    {/* PARCIAL */}
                                    <td className="p-1 text-right font-mono font-semibold bg-amber-950/10">
                                        {node.tipo_fila === 'detalle' ? (
                                            <span className={`text-xs ${node._fromRemuneraciones ? 'text-emerald-300' : 'text-slate-200'}`}>
                                                S/. {fmt(node.parcial)}
                                            </span>
                        ...
                        <tfoot>
                            <tr className="bg-slate-800/80 border-t-2 border-amber-800/50">
                                <td colSpan={7} className="p-3 text-right text-xs font-bold tracking-widest text-slate-300 uppercase">
                                    TOTAL GASTOS VARIABLES
                                </td>
                                <td className="p-3 text-right font-mono font-bold text-amber-400 text-xs bg-amber-950/30">
                                    S/. {fmt(total)}
                                </td>
                                <td />
                            </tr>
            ...
            <div className="flex items-center justify-between border-t border-slate-700 bg-slate-800/40 p-3 backdrop-blur-sm">
                <div className="flex items-center gap-1 flex-wrap">
                    <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg bg-amber-900/30 px-3 py-1.5 text-xs font-bold text-amber-300 transition-all hover:bg-amber-900/50 border border-amber-800/40"
                    >
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg bg-sky-900/40 px-3 py-1.5 text-xs font-bold text-sky-200 transition-all hover:bg-sky-900/60 border border-sky-800/40 shadow-lg"
                        >
                        ...
                        {showTemplates && (
                            <>
                                ...
                                <div className="absolute bottom-full left-0 mb-2 z-50 w-72 rounded-xl border border-slate-700 bg-slate-800 p-1 shadow-2xl animate-in fade-in slide-in-from-bottom-2">
                                    <div className="px-3 py-2 text-xs font-bold tracking-widest text-slate-500 uppercase flex items-center gap-1">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1 rounded-lg px-5 py-2 text-xs font-bold text-white shadow-lg transition-all ${isDirty
            ...
            {contextMenu && (
                <>
                    ...
                    >
                        ...
                        <button
                            className="w-full px-4 py-1.5 text-left hover:bg-slate-700 hover:text-slate-200 flex items-center gap-1"
                            onClick={() => { addNode(contextMenu.index, 'detalle'); setContextMenu(null); }}
                        >
                            Nuevo Ítem (detalle)
                        </button>
                        <button
                            className="w-full px-4 py-1.5 text-left hover:bg-emerald-900/30 hover:text-emerald-300 flex items-center gap-1"
