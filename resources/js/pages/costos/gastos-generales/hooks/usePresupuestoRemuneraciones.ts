...
import { normalizarRemuneracionesParaGuardar, obtenerMensajeValidacion } from '../lib/remuneracionesPayload';
...
export function usePresupuestoRemuneraciones({
    ...
    useEffect(() => {
        if (subsection !== 'remuneraciones' || loadedProjectId.current === projectId) {
            return;
        }
    ...
    const saveRemuneracion = useCallback(async (data: any) => {
        try {
            const rows = normalizarRemuneracionesParaGuardar(data);
            ...
            throw new Error(obtenerMensajeValidacion(error));
        }
    }, [projectId, setDirty]);
