...
function updateNode(node: FormulaNode, id: string, update: (node: FormulaNode) => FormulaNode): FormulaNode {
    if (node.id === id) return update(node);
    return { ...node, children: node.children.map((child) => updateNode(child, id, update)) };
}
function detachChild(node: FormulaNode, id: string): { node: FormulaNode; detached: FormulaNode | null } {
...
export function FormulaPolinomicaBuilder({ parentMap, codeToDesc, sortedCodes, persistedMonomios, isLoading = false, onMonomiosChange }: Props) {
    ...
    const usedCodes = useMemo(() => new Set(allNodes.map((node) => node.code)), [allNodes]);
    const totalInsumo = useMemo(() => Array.from(parentMap.values()).reduce((sum, value) => sum + value, 0), [parentMap]);
    const available = useMemo(() => sortedCodes.filter((code) => !usedCodes.has(code) && (parentMap.get(code) ?? 0) > 0).map((code) => {
        const calculated = (parentMap.get(code) ?? 0) / totalInsumo;
        return { id: `i-${code}`, code, descripcion: codeToDesc.get(code) ?? `Índice ${code}`, coefCalculado: calculated, coefDefinido: Number(calculated.toFixed(3)), children: [] } satisfies FormulaNode;
    ...
    const saveCoef = (id: string, raw: string) => {
        ...
        if (Number.isFinite(value)) setMonomios((current) => current.map((item) => ({ ...item, root: updateNode(item.root, id, (node) => ({ ...node, coefDefinido: Math.max(0, Math.min(1, value)) })) })));
        setEditingCoef(null);
    };
    const extract = (nodeId: string) => setMonomios((current) => {
        let detached: FormulaNode | null = null;
    ...
    const renderNode = (node: FormulaNode, monomio: FormulaMonomio, depth: number, root: boolean): React.ReactNode => {
        ...
        return <React.Fragment key={node.id}>
            <tr onDragOver={(event) => { const reorderSourceId = reorderDragIdRef.current; if (reorderSourceId && reorderSourceId !== node.id) { if (!siblingIds.includes(reorderSourceId)) { event.dataTransfer.dropEffect = 'none'; return; } event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const rect = event.currentTarget.getBoundingClientRect(); setReorderPlacement(event.clientY < rect.top + rect.height / 2 ? 'before' : 'after'); setReorderOverId(node.id); return; } if (!activeSourceId || activeSourceId === node.id) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = canReceive ? 'move' : 'none'; setDragOverId(node.id); }} onDrop={(event) => reorderDragIdRef.current ? dropReorder(event, node.id) : drop(event, node.id)} className={`border-b border-slate-700/60 ${root ? 'border-t-2 border-t-slate-600 bg-slate-800' : 'bg-slate-900'} ${dragOverId === node.id ? 'ring-2 ring-inset ring-sky-500' : ''} ${reorderOverId === node.id ? `ring-2 ring-inset ${reorderPlacement === 'before' ? 'ring-emerald-400' : 'ring-teal-500'}` : ''} ${selectedNodeId === node.id ? 'bg-violet-950/60 ring-1 ring-inset ring-violet-500' : ''}`}>
                ...
                <td className="py-1.5 pr-2 text-right font-mono text-slate-400">{nodeTotal > 0 ? '100.0' : '0.0'}</td>
            ...
            <tr className="border-b border-slate-800 bg-slate-950">
                ...
                <td className="py-1.5 pr-1 text-slate-300" style={{ paddingLeft: 28 + depth * 22 }}><div className="flex items-center gap-1.5"><span className="rounded bg-slate-800 px-1 py-0.5 font-mono text-[9px] text-sky-400">{node.code}</span><span>{node.descripcion}</span><span className="text-[8px] uppercase text-slate-600">índice</span></div></td>
                <td className="p-1 text-center text-slate-700">—</td>
                <td className="py-1.5 pr-2 text-right font-mono text-slate-300">{editingCoef === node.id ? <input autoFocus defaultValue={fmt(node.coefDefinido)} className="w-20 rounded border border-emerald-500 bg-slate-900 px-1 text-right" onBlur={(event) => saveCoef(node.id, event.target.value)} /> : <button type="button" onClick={() => setEditingCoef(node.id)}>{fmt(node.coefDefinido)}</button>}</td>
                <td className="py-1.5 pr-2 text-right font-mono text-slate-500">{nodeTotal > 0 ? ((node.coefDefinido / nodeTotal) * 100).toFixed(1) : '0.0'}</td>
                <td />
            </tr>
            {isExpanded && node.children.map((child) => renderNode(child, monomio, depth + 1, false))}
        </React.Fragment>;
    };

    return <div className="flex min-h-0 flex-1 flex-col overflow-hidden text-[11px]">
        <div className="flex shrink-0 items-center gap-2 border-b border-slate-700 bg-slate-900 px-3 py-1.5"><span className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Fórmula Polinómica</span><span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${monomios.length > MAX_MONOMIOS ? 'bg-amber-900/40 text-amber-300' : 'bg-slate-800 text-slate-400'}`}>{monomios.length}/{MAX_MONOMIOS} monomios</span><span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ${totalOk ? 'bg-emerald-900/40 text-emerald-300' : 'bg-amber-900/40 text-amber-300'}`}>Σ = {fmt(total)}</span><div className="flex-1" /><button type="button" onClick={() => { setMonomios(initial()); setExpanded(new Set()); }} className="flex items-center gap-1 rounded px-2 py-0.5 text-[10px] text-slate-400 hover:bg-slate-700"><RefreshCw size={10} /> Restablecer sin agrupar</button></div>
        {monomios.length > MAX_MONOMIOS && <div className="flex items-center gap-1.5 border-b border-amber-900/50 bg-amber-950/30 px-3 py-1.5 text-[9px] text-amber-400"><AlertTriangle size={11} /> Máximo normativo: {MAX_MONOMIOS}. Cada nodo admite hasta dos hijos.</div>}
