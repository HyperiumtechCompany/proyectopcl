...
function uniformGuides(cellCount: number): number[] {
    return normalizeGuideBoundaries(
        undefined,
        Math.max(1, Math.round(cellCount)) - 1,
    ).slice(1, -1);
}
...
export const createUiSlice: EditorSlice<UiSlice> = (set, get) => ({
    setTool: (tool) =>
        set((s) => ({
            ui: {
                ...s.ui,
                activeTool: tool,
                sidebarTab: ELECTRICAL_TOOLS.has(tool)
                    ? 'legend'
                    : s.ui.sidebarTab,
            },
        })),
    ...
    toggleElectricalLayer: (group) =>
        set((s) => {
                ...
                return {
                    ui: {
                        ...
                        electricalLayerVisibility: {
                            ...s.ui.electricalLayerVisibility,
                            [group]: true,
                        },
                    },
                };
            ...
            const belongsToGroup = (id: string): boolean => {
                ...
                if (
                    group === 'fixtures' &&
                    scene.fixtures.some((item) => item.id === id)
                )
                    return true;
                if (
                    group === 'switches' &&
                    (scene.lightSwitches ?? []).some((item) => item.id === id)
                )
                    return true;
                const device = (scene.electricalDevices ?? []).find(
                    (item) => item.id === id,
                );
                if (device)
                    return (
                        group ===
                        (device.type.startsWith('outlet_')
                            ? 'outlets'
                            : 'panels')
                    );
            ...
            return {
                ui: {
                    ...
                    selectedId:
                        s.ui.selectedId && belongsToGroup(s.ui.selectedId)
                            ? null
                            : s.ui.selectedId,
                    selectedFixtureIds: s.ui.selectedFixtureIds.filter(
                        (id) => !belongsToGroup(id),
                    ),
                },
            };
        }),
            ...
            return {
                ui: {
                    ...
                    hiddenElectricalIds: hiding
                        ...
                        : s.ui.hiddenElectricalIds.filter(
                              (itemId) => itemId !== id,
                          ),
                    selectedId:
                        hiding && s.ui.selectedId === id
                            ? null
                            : s.ui.selectedId,
                    selectedFixtureIds: hiding
                        ? s.ui.selectedFixtureIds.filter(
                              (itemId) => itemId !== id,
                          )
    ...
    clearFixtureSelection: () =>
        set((s) => ({
            ui: { ...s.ui, selectedFixtureIds: [], selectedId: null },
        })),
    ...
    set3DView: (visible) =>
        set((s) => ({ ui: { ...s.ui, show3DView: visible } })),
    toggleRoof: () =>
    ...
    setSwitchTemplate: (template) =>
        set((s) => ({
            ui: {
                ...s.ui,
                switchTemplate: { ...s.ui.switchTemplate, ...template },
            },
        })),
    setWireTemplate: (template) =>
        set((s) => ({
            ui: {
                ...s.ui,
                wireTemplate: { ...s.ui.wireTemplate, ...template },
            },
        })),
    ...
    setFixtureGridGuide: (axis, index, value) =>
        set((s) => {
            ...
            const MIN_GAP = 0.02;
            const lowerBound =
                index === 0 ? MIN_GAP : guides[index - 1] + MIN_GAP;
            const upperBound =
                index === guides.length - 1
                    ? 1 - MIN_GAP
                    : guides[index + 1] - MIN_GAP;
            guides[index] = Math.min(upperBound, Math.max(lowerBound, value));

            const nextEditor: FixtureGridGuideEditorState = {
                ...editor,
                [key]: guides,
            };
