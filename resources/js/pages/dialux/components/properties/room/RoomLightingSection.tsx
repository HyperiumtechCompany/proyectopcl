...
import type { NormativeLeafOption, NormativeStandard } from '@/pages/dialux/hooks/roomLighting';
import { NORMATIVE_LABELS, type RoomLightingInputs } from '@/pages/dialux/hooks/roomLighting';
...
export function RoomLightingSection({
    ...
    return (
        <SectionWrapper icon={<Zap size={12} className="text-yellow-400" />} label="Iluminación">
            ...
            {room.normativeCategory && (
                <SelectField
                    ...
                    options={normActivities.map((a) => ({ value: a.activity, label: a.activity }))}
                    onChange={(val) => {
            ...
            <EditField
                ...
                max={2000}
                step={10}
                onChange={(value) => onUpdate({ illuminanceLux: value, norma: value })}
            />
            <div className="flex items-center justify-between">
                <PropField label="Luminarias" value={`${fixturesInRoom.length}`} />
            ...
            <PropField
                label="Lm detectados"
                value={inputs.detectedFixtureLumens ? `${inputs.detectedFixtureLumens.toLocaleString()} lm` : '—'}
            />
            <PropField label="Lm requeridos" value={`${inputs.lumensRequired.toFixed(0).toLocaleString()} lm`} />
            <PropField label="Cant. óptima" value={`${inputs.exactQuantity.toFixed(2)}`} />
            <PropField label="Cant. simetría" value={`${inputs.roundedQuantity}`} />
