...
export const StatusBar = memo(function StatusBar() {
    ...
    return (
        <footer
            id="dialux-statusbar"
            className="h-6 px-4 bg-white dark:bg-[#0e1015] border-t border-gray-300 dark:border-gray-800/60 flex items-center gap-4 text-[10px] text-gray-600 dark:text-gray-600 shrink-0 select-none"
        >
            <span>Zoom: <span className="text-gray-500 dark:text-gray-500">{(zoom * 100).toFixed(0)}%</span></span>
            <span>Escala: <span className="text-gray-500 dark:text-gray-500">1 m = 60 px</span></span>
            <span>
                <span className="text-gray-500 dark:text-gray-500">{roomCount}</span> recintos ·{' '}
                <span className="text-gray-500 dark:text-gray-500">{fixCount}</span> luminarias
            </span>
            {result && (
            ...
            <span className="text-gray-700 dark:text-gray-700">Motor JS · Babylon.js ready</span>
        </footer>
    );
});
