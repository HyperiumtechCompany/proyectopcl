...
export function SiteCanvas2D({ editor }: Props) {
    ...
    const cadPlan = useSiteCadPlan(editor.projectId, editor.generalModuleId);
    /** Con el plano CAD vivo debajo, la imagen PNG sobra (y lo taparia). */
    ...
    // Mantiene la camara del motor CAD alineada con el viewBox del SVG. Se
    // corre en `useLayoutEffect` para que el plano se reubique en el MISMO
    // commit en que el SVG se repinta: con `useEffect` (post-paint) el plano
    // quedaba un frame atras y se veia 'nadar' respecto de la grilla.
    useLayoutEffect(() => {
        if (!cadPlanActive) return;
        const width = svgRef.current?.clientWidth ?? 0;
        if (width <= 0) return;
        cadPlan.syncCamera(viewBox, width);
    }, [cadPlanActive, viewBox, cadPlan]);
    ...
    return (
        <div className="relative h-full min-h-105 w-full bg-[radial-gradient(circle,#94a3b833_1px,transparent_1px)] bg-size-[20px_20px] dark:bg-[radial-gradient(circle,#47556955_1px,transparent_1px)]">
            {/* Motor CAD: dibuja el plano vectorial DEBAJO del overlay SVG.
                Su camara la alinea `syncCamera` con el viewBox, asi que el
                SVG sigue mandando el pan/zoom. `pointer-events-none`: todos
                los gestos los sigue manejando el SVG de arriba. */}
            <div
            ...
            <svg
                ...
                className="relative h-full min-h-[420px] w-full touch-none select-none"
                onPointerDown={(event) => {
            ...
            >
                ...
                {siteData.importedPlan?.visible && !cadPlanActive && (
                    <ImportedPlanImage
