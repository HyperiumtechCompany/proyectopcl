...
export function isFinitePoint(p: WorldPoint | null | undefined): p is WorldPoint {
...
export function sanitizePolygon(vertices: WorldPoint[]): WorldPoint[] {
    ...
    for (const v of vertices) {
        ...
        if (prev && Math.abs(prev.x - v.x) < EPSILON && Math.abs(prev.y - v.y) < EPSILON) {
            continue;
        }
        out.push(v);
    }
...
export function pointInPolygon(point: WorldPoint, vertices: WorldPoint[]): boolean {
    const ring = sanitizePolygon(vertices);
...
export function distancePointToSegment(p: WorldPoint, a: WorldPoint, b: WorldPoint): number {
    const dx = b.x - a.x;
    const dy = b.y - a.y;
...
export function distanceToPolygonEdge(point: WorldPoint, vertices: WorldPoint[]): number {
    const ring = sanitizePolygon(vertices);
    if (ring.length === 0 || !isFinitePoint(point)) return Infinity;
    if (ring.length === 1) return Math.hypot(point.x - ring[0].x, point.y - ring[0].y);
    let min = Infinity;
    for (let i = 0; i < ring.length; i++) {
        const a = ring[i];
        const b = ring[(i + 1) % ring.length];
...
function segmentsIntersect(p1: WorldPoint, p2: WorldPoint, p3: WorldPoint, p4: WorldPoint): boolean {
...
export function validatePolygon(vertices: WorldPoint[]): PolygonValidation {
    ...
    if (ring.length >= 4 && isSelfIntersecting(ring)) {
        warnings.push('El polígono se autointerseca; el área calculada puede no ser la esperada.');
    }
