...
class UpdateElectricalNetworkRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'data.settings.faultClearingTimeS' => ['nullable', 'numeric', 'gt:0', 'max:5'],
            'data.nodes' => ['required', 'array', 'max:500'],
            'data.nodes.*.id' => ['required', 'string', 'distinct', 'max:100'],
            'data.nodes.*.type' => ['required', Rule::in(['service', 'meter', 'ats', 'generator', 'ups', 'main_panel', 'module_panel_port', 'site_panel'])],
            ...
            'data.nodes.*.origin' => ['nullable', Rule::in(['site'])],
            'data.nodes.*.siteElementId' => ['nullable', 'string', 'max:100'],
            'data.nodes.*.supplyMode' => ['nullable', Rule::in(['own'])],
            'data.nodes.*.phases' => ['nullable', 'integer', Rule::in([1, 3])],
            'data.nodes.*.nominalVoltageV' => ['nullable', 'numeric', 'gt:0'],
            'data.nodes.*.simultaneityFactor' => ['nullable', 'numeric', 'gt:0', 'max:1'],
            'data.nodes.*.transformerKva' => ['nullable', 'numeric', 'gt:0'],
            'data.nodes.*.supplyReservePercent' => ['nullable', 'numeric', 'min:0', 'max:200'],
            'data.nodes.*.transformerUkPercent' => ['nullable', 'numeric', 'gt:0', 'max:20'],
            'data.nodes.*.upstreamShortCircuitMva' => ['nullable', 'numeric', 'gt:0'],
            'data.nodes.*.breakingCapacityKa' => ['nullable', 'numeric', 'gt:0'],
            'data.nodes.*.phase' => ['nullable', Rule::in(['R', 'S', 'T'])],
            ...
            'data.edges.*.lengthMode' => ['required', Rule::in(['manual', 'plan', 'combined', 'site'])],
            'data.edges.*.origin' => ['nullable', Rule::in(['site'])],
            'data.edges.*.siteCircuitId' => ['nullable', 'string', 'max:100'],
